...
class SocialProvider extends ChangeNotifier {
  ...
  void subscribeToSocialAccounts(String brokerId) {
    ...
    _socialSubscription = SupabaseConfig.client
        ...
        .onPostgresChanges(
          ...
          callback: (payload) {
            debugPrint('[SocialProvider] Realtime Postgres Change Received (${payload.eventType}): new=${payload.newRecord}, old=${payload.oldRecord}');
    ...
    _socialSubscription!.subscribe((status, [error]) {
      debugPrint('[SocialProvider] Realtime Subscription Status for social_accounts: $status ${error != null ? ", error: $error" : ""}');
    });
  }
  ...
  void _handleRealtimePayload(PostgresChangePayload payload) {
    debugPrint('[SocialProvider] Realtime Postgres Change Received (${payload.eventType}) for broker: $_currentBrokerId');
  ...
  Future<void> fetchInitialConnections(String brokerId, {bool forceShimmer = false}) async {
  ...
  Future<SocialPostModel?> enablePostAutomation(SocialPostModel post, String brokerId) async {
    try {
      ...
      final payload = <String, dynamic>{
        ...
        if (post.propertyId?.id != null)
          'property_id': post.propertyId!.id,
      };
      ...
      AppToast.showSuccess(
        'Automation Enabled',
        'Automation is now active for this post.',
      );
      return updatedPost;
    } catch (e) {
      ...
      if (errStr.contains('duplicate key') || errStr.contains('unique_broker_platform_post') || errStr.contains('23505')) {
        try {
          final targetPostId = (post.postId != null && post.postId!.isNotEmpty)
              ? post.postId!
              : (post.platformPostId ?? post.id ?? '');
          ...
          if (existing != null) {
            ...
            AppToast.showSuccess(
              'Automation Active',
              'Automation is already active for this post.',
            );
            return updatedPost;
          }
        } catch (fetchErr) {
      ...
      AppToast.showError('Error', 'Could not enable automation. Please try again.');
      return null;
    }
  }
  ...
  Future<SocialPostModel?> disablePostAutomation(SocialPostModel post, String brokerId) async {
    try {
      ...
      AppToast.showSuccess(
        'Automation Disabled',
        'Automation has been turned off for this post.',
      );
      return updatedPost;
    } catch (e) {
      debugPrint('[SocialProvider] Error disabling post automation: $e');
      AppToast.showError('Error', 'Could not disable automation. Please try again.');
      return null;
    }
  }
  ...
  Future<bool> publishInstagramPost({
    ...
    try {
      ...
      for (int i = 0; i < medias.length; i++) {
        ...
        final baseName = media.name
            .replaceAll(RegExp(r'[^a-zA-Z0-9._-]'), '_')
            .replaceAll(RegExp(r'\.(jpg|jpeg|png|webp|mp4)$', caseSensitive: false), '');
        final uniqueName = '${DateTime.now().millisecondsSinceEpoch}_$baseName$ext';
        
        final mediaUrl = await uploadAsset(uniqueName, media.bytes, mimeType: mimeType);
        ...
        try {
          ...
          debugPrint('[SocialProvider] CRITICAL: The uploaded asset URL ($mediaUrl) is NOT publicly accessible! '
              'Meta Graph API will reject it with "Invalid parameter". Please verify your storage bucket settings. Error: $testErr');
        }

        String? coverUrl;
        if (media.type == 'video' && media.thumbnailBytes != null) {
          _updatePublishingState(true, 'Uploading video covers (${i + 1}/${medias.length})...', 0.1 + (i / medias.length) * 0.3);
          ...
          coverUrl = await uploadAsset(coverName, media.thumbnailBytes!, mimeType: 'image/jpeg');
          
          try {
            final testRes = await Dio().head(coverUrl);
            debugPrint('[SocialProvider] Public Cover URL accessibility check for $coverUrl: Status ${testRes.statusCode}');
          } catch (testErr) {
            debugPrint('[SocialProvider] CRITICAL: The cover URL ($coverUrl) is NOT publicly accessible! Error: $testErr');
          }
        }
  ...
  Future<bool> publishFacebookPost({
    ...
    try {
      ...
      for (int i = 0; i < medias.length; i++) {
        ...
        // Sanitize the filename to prevent URL space/character formatting issues with Meta Graph API
